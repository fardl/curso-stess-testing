...
escenario_arima_shock <- function(modelo_arima,
  ...
  if (usar_fechas) {
    ...
    if (!is.na(med_days) && med_days >= 27 && med_days <= 32) {
      fechas_fc <- seq(lubridate::`%m+%`(ultima_fecha, lubridate::months(1)), by = "1 month", length.out = h)
    } else if (!is.na(med_days) && med_days >= 80 && med_days <= 100) {
      fechas_fc <- seq(lubridate::`%m+%`(ultima_fecha, lubridate::months(3)), by = "3 months", length.out = h)
    } else if (!is.na(med_days) && med_days >= 360 && med_days <= 370) {
      fechas_fc <- seq(lubridate::`%m+%`(ultima_fecha, lubridate::years(1)), by = "1 year", length.out = h)
    } else {
      fechas_fc <- ultima_fecha + round(med_days) * seq_len(h)
    }
  }
