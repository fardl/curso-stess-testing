...
# 5. Escenarios condicionales: baseline, adverso y muy adverso ----
...
restricciones_baseline <- tibble(
  crecimiento   = c(3.0, 3.2, 3.4, 3.5, 3.5),
...
historico_grafico <- macro %>%
  ...
  mutate(escenario = "histórico")

escenarios_grafico <- escenarios_finales %>%
  mutate(fecha = as.Date(fecha)) %>%
  dplyr::select(fecha, escenario, variable, valor)
...
datos_grafico <- bind_rows(
  ...
  mutate(
    escenario = factor(
      escenario,
      levels = c("histórico", "Baseline VAR", "adverso", "muy_adverso")
    )
  )
